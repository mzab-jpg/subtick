...
- **M1 — Reader RSS warming:** The Reader warms only the next two queued publisher feeds, one at a time, retaining their raw parsed RSS data in app-process memory. Repeat articles and later Reader visits reuse the first publisher download. It never pre-cleans article HTML; cleaning happens only when a person opens that article, protecting scroll responsiveness. Nothing is written to disk, and Android discards the raw cache automatically when the app closes. This avoids the old parallel burst and does not move article bodies to Tangent's backend.
