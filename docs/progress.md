...
- ✅ **Android native RSS preloader** — Local Expo module `modules/tangent-rss-parser` downloads/parses RSS/Atom outside Reader JavaScript. Lookahead retains raw XML for up to 16 ordinary-sized feeds (5 MB cache allowance per feed) and extracts raw bodies for exactly the next five Reader targets, not every entry from each publisher feed. At most two speculative future targets prepare concurrently, so one slow publisher does not hold every later target behind it. On opening an article, native code returns its already prepared raw body when available; JavaScript still sanitises only the displayed article. The Dashboard-tapped/current article and every previously shown Reader article are fixed. Within only the next five unseen cards, completed preparations may be displayed before still-loading cards; their original order remains the tie-breaker and feed membership never changes. A larger legitimate feed is stream-parsed directly to the selected item and is not cached, so the cache allowance never creates a Reader error or excludes an article. Reader maintains a rolling five-upcoming-article target buffer (articles 2–6 on opening article 1). No cleaned HTML is preloaded or stored on disk. iOS remains on the JavaScript fallback until its matching native module is implemented.
- ✅ **Instant-swipe reliability** — A deliberate 200 ms finger pause before release cancels an edge swipe without navigation or behaviour recording. Separately, behavior sync no longer begins at a batch boundary during active Reader use. Fast article transitions keep the existing Reader visible and show no spinner; only an unresolved request after 180 ms receives a small non-interactive overlay. Stale rapid-swipe loads cannot overwrite the newest requested article.
- ✅ **Reader-safe behavior sync** — Reaching the 20-event batch size and queue replenishment no longer start a Cloud Function upload during active reading. Events stay in the durable on-device queue, then sync on Reader exit, reconnect, or another lifecycle flush.
