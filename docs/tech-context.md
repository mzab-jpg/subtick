...
- `src/services/startupCache.ts` — UID-bound, non-sensitive AsyncStorage route snapshot and 24-hour Dashboard-card cache. It stores no credentials/tokens and is only a display shortcut after Firebase has restored the same UID; cloud profile/functions remain authoritative.
- `src/services/dashboardFeedCache.ts` — UID-scoped Dashboard cache mirrored to `startupCache`; remounts/restarts restore current unread cards, Reader removes only genuinely opened cards, and background freshness is staged without replacing visible cards.
- `src/services/initialDashboardFeed.ts` — Shares the first ranked-feed request between onboarding and Dashboard so a category-save handoff does not create two requests.
...
- `src/components/StartupScreen.tsx` — React-level top-left system-font `TANGENT` / typed red `sapere aude|` startup identity shown after the Android native splash while auth/profile initialization runs. It remains until the returning account's initial cards are prepared, preventing a Home loading flash. `LoadingCursor.tsx` supplies the matching red `Loading|` wait state on Home and Reader. Both use React Native core `Animated`, introduce neither a web font nor a package. The native `assets/splash-icon.png`/`app.json` splash is intentionally unchanged during this trial and must be redesigned separately before release if a seamless handoff is required.
...
- `ReaderScreen.tsx` is a 430-line orchestrator delegating to 3 hooks + 2 components; Android maintains a rolling five-upcoming-article native RSS buffer using at most two speculative workers, retains only raw publisher data in process memory, and sanitizes article HTML only when displayed. The Dashboard-tapped/current article is fixed and the ranked Reader order remains sequential. Failed future lookahead cards are omitted only from the current Reader and mounted Dashboard cache, not History/persistent seen state. Active and lookahead requests for one exact article share one native operation, preventing a duplicate large-feed scan.
